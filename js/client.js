proxy = {
	url: "http://elloworld.noip.me/forums",
	get: function(ab, callback) {
		$.get(ab, callback);
	},
	post: function(a, b, callback) {
...
window.speed = 0.75;
if ($.cookie('anim_speed')) window.speed = Math.abs($.cookie('anim_speed')); else $.cookie('anim_speed', window.speed, {expires: 365*3});
(function($) {
	...
	Config.origindomain = '/pb/';

	// `defaultserver` specifies the server to use when the domain name in the
	// address bar is `Config.origindomain`.
	Config.defaultserver = {
		id: 'frost', //this is for replays mainly, just put a server id that is registered
	...
	var User = this.User = Backbone.Model.extend({
		...
		rename: function(name) {
			...
			*/
			proxy.get(proxy.url + '/POKEBATTLE.php?action=name&name=' + name, function(data) {
				var events = [
					"unregistered",
					"registered",
					"correct password",
				...
				if (event == "unregistered" || event == "correct password") app.send('/trn ' + name);
				if (event == "incorrect password" || event == "registered") {
					var obj = {username: name};
					if (event == "incorrect password") obj.error = 'Wrong password.';
					app.addPopup(LoginPasswordPopup, obj);
				}
			});
		},
		...
		receiveChallenge: function(attrs) {
			if (attrs.challenge) {
				...
				var self = this;
				getProxy(query, Tools.safeJSON(function(data) {
		...
		setPersistentName: function(name) {
			if (!name || name.substr(0, 6) == "Guest ") return;
	...
	var App = this.App = Backbone.Router.extend({
		...
		initializeConnection: function() {
			if (1 == 0 && (document.location.hostname !== Config.origindomain) && !Config.testclient) {
				// Handle *.psim.us.
		...
		connect: function() {
			...
			this.socket.onopen = function() {
				...
				if ($.cookie('showdown_username')) app.send('/trn ' + $.cookie('showdown_username'));
			};
		...
		focusRoom: function(id) {
			if (id) app.focusedRoom = id;
			if (id == 'lobby') this.focusRoom('');
			...
			if (this.curSideRoom !== room) {
				if (this.curRoom) {
					if (this.curRoom.el.id != "mainmenu") {
						this.curRoom.hide();
					}
					this.curRoom = null;
				}
		...
		updateLayout: function() {
			...
			this.dismissPopups();
			function updateLogSizePB() {
				//make room for navigation
				$('body').removeAttr('style');
				$('body').css({
					'margin-left': '150px',
					'width': ($('body').width() - 150) + 'px',
				})
				//fix userlist to make it accurately look like PB
				$(".userlist").show();
				var chatSize = $(".ps-room.ps-room-light").width() - 146;
				if (chatSize < 300) {
					//chat is too small so hide user list
					chatSize = chatSize + 146;
					$('.userlist').hide();
				}
				$(".chat-log, .tournament-wrapper").width(chatSize);
				$(".chat-log-add").width(chatSize - 9);
				$('.mainmenu').height($('body').height() - 115);
			}
			if (!this.sideRoom) {
	...
	var Topbar = this.Topbar = Backbone.View.extend({
		...
		updateTabbar: function() {
			//pokebattle
			$('.nav_battles, .nav_rooms').empty();
			function pokeBattleAddTab(room) {
				if (room.id == '') return;
				var el = $("#tab" + room.id);
				if (el.length == 0) {
					//add tab
					var title = room.title;
					if (!title) title = room.id;
					var insides = '<li id="tab' + room.id + '" class="nav_item fake_link" onclick="app.focusRoom(\'' + room.id + '\');">' + title + '</li>';
					if (room.id.substr(0, 7) == "battle-") {
						insides = '<li id="tab' + room.id + '" class="nav_item fake_link" onclick="app.focusRoom(\'' + room.id + '\');">';
						insides += '<div class="nav_meta"><div class="close" onclick="app.leaveRoom(\'' + room.id + '\');">×</div></div>';
						insides += title;
						insides += '</li>';
						$(".nav_battles").append(insides);
					} else $(".nav_rooms").append(insides);
				}
			}
			//normal
			...
			var buf = '';
			buf = '<ul><li><a class="button'+(curId===''?' cur':'')+(app.rooms['']&&app.rooms[''].notifications?' notifying':'')+'" href="'+app.root+'"><i class="icon-home"></i> <span>Home</span></a></li>';
			...
			//pokebattle
			var showbattles = "hide";
			if ($(".nav_battles").html()) showbattles = "show";
			$(".header_battles, .nav_battles")[showbattles]();
			$(".active").removeClass("active");
			if (!app.focusedRoom) app.focusedRoom = 'lobby';
			$("#tab" + app.focusedRoom).addClass("active");
		},
		...
		constructor: function(data) {
			...
			if (this.type === 'normal') {
				...
				if (this.position === 'right') {
					...
					$el.css('left', offset.left + this.sourceEl.outerWidth() - 456);
	...
	var OptionsPopup = this.OptionsPopup = Popup.extend({
		...
		update: function() {
			...
			buf += '<hr />';
			buf += '<p><label class="optlabel">Animation Speed: <select id="speed" onchange="window.speed = Math.abs(this.value);$.cookie(\'anim_speed\', window.speed, {expires: 365*3});"><option value="0.5">Very Fast</option><option value="0.75">Fast</option><option value="1">Medium</option><option value="1.25">Slow</option><option value="1.5">Very Slow</option></select></label></p>';
			...
			if (app.user.get('named')) {
				...
				if (registered && (registered.userid === app.user.get('userid'))) {
					...
					buf += '<a href="http://elloworld.noip.me/forums/entry/register?Target=discussions" target="_BLANK"><button>Register</button></a> ';
				}
