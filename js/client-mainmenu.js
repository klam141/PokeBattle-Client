(function($) {
	...
	var MainMenuRoom = this.MainMenuRoom = this.Room.extend({
		...
		initialize: function() {
			...
			buf += '<div class="menugroup"><p><button class="button" name="joinRoom" value="teambuilder" id="openTeambuilder">Teambuilder</button></p>';
			if (app.down) {
				buf += '<p><button class="button disabled" name="joinRoom" value="ladder" disabled>Ladder</button></p>';
			} else {
				buf += '<p><button class="button" name="joinRoom" value="ladder" id="openLadder">Ladder</button></p>';
			}
			buf += '<p><button class="button" name="credits">Credits</button></p></div></div>';

			if (!app.down) buf += '<div class="menugroup"><p><button class="button" name="roomlist" id="openBattleList">Watch a battle</button></p></div>';
		...
		pmsShowing: 0,
		pmsSpacing: new Array(),
		pmWidth: 270,
		updateConvos: function(name) {
			//update side bar
			//pokebattle
			if (name) this.conversations[toId(name)] = name;
			$(".nav_messages").empty();
			for (var uid in this.conversations) {
				var insides = '';
				insides += '<li class="nav_item fake_link" onclick="if ($(event.target).hasClass(\'close\')) {return false;}app.rooms[\'\'].openPM(\'' + this.conversations[uid].split("'").join("\\'") + '\');">';
				insides += '<div class="nav_meta"><div class="close" onclick="delete app.rooms[\'\'].conversations[\'' + uid + '\'];app.rooms[\'\'].updateConvos();app.rooms[\'\'].closePM(\'' + uid + '\');">×</div></div>';
				insides += Tools.escapeHTML(this.conversations[uid]);
				insides += '</li>'
				$(".nav_messages").append(insides);
			}
			var showpms = "hide";
			if ($(".nav_messages").html()) showpms = "show";
			$(".header_messages, .nav_messages")[showpms]();
		},
		openPM: function(name, dontFocus) {
			//pokebattle
			this.updateConvos(name);
			
			var self = this;
			function pmShow(minimize) {
				var slot = undefined;
				for (var i in self.pmsSpacing) {
					if (!self.pmsSpacing[i]) {
						slot = i;
						self.pmsSpacing[i] = toId(name);
						break;
					} 
				}
				if (slot === undefined) {
					slot = self.pmsShowing;
					self.pmsShowing++;
					self.pmsSpacing.push(toId(name));
				}
				$pmWindow.css({
					'width': self.pmWidth + 'px',
					left: (self.pmWidth * slot) + 'px'
				});
				if (dontFocus && !$pmWindow.data('minimized') && minimize) $pmWindow.find('.minimizebutton').click();
			}
			...
			if (!$pmWindow.length) {
				...
				if ($pmWindow.css('display') == 'none') pmShow(true);
				$pmWindow.show();
				if (!dontFocus) {
					var $chatFrame = $pmWindow.find('.pm-log');
					var $chat = $pmWindow.find('.inner');
		...
		closePM: function(e) {
			...
			var self = this;
			function removeFromSpacing() {
				self.pmsShowing--;
				var moveLeft = 0;
				for (var i in self.pmsSpacing) {
					if (self.pmsSpacing[i] == userid) {
						self.pmsSpacing.splice(i, 1);
						moveLeft = i;
					}
				}
				for (var i = moveLeft; i < self.pmsSpacing.length; i++) $('.pm-window-' + self.pmsSpacing[i]).css('left', (self.pmWidth * i) + 'px');
			}
			removeFromSpacing();
			var $pmWindow;
			if (!userid) {
