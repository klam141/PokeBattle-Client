(function(exports, $) {
	...
	var TeambuilderRoom = exports.TeambuilderRoom = exports.Room.extend({
		...
		updateTeamList: function() {
			...
			if (!teams.length) {
				...
				for (var i=0; i<teams.length+1; i++) {
					...
					buf += '<li><button name="edit" value="'+i+'" class="buttonteam">'+formatText+'<h2>'+Tools.escapeHTML(team.name)+'</h2><small>';
					buf += '<div class="team-pbv">PBV: ' + pbvs(team.team).total + '/1000</div>';
					for (var j=0; j<team.team.length; j++) {
						var name = team.team[j].name;
						buf += '<span class="conicon" style="' + Tools.getIcon(name) + '"></span>';
					}
		...
		renderTeambar: function() {
			...
			var pbv = pbvs(this.curTeam.team);
			for (var i=0; i<this.curTeam.team.length; i++) {
				...
				if (!set.name) {
					...
					buf += '<button disabled="disabled" class="pokemon"><div class="builder-pbv">'+pbv.mons[i]+'</div>'+pokemonicon+Tools.escapeHTML(set.name || '<i class="icon-plus"></i>')+'</button> ';
				} else {
					buf += '<button name="selectPokemon" value="'+i+'" class="pokemon"><div class="builder-pbv">'+pbv.mons[i]+'</div>'+pokemonicon+Tools.escapeHTML(set.name)+'</button> ';
				}
			}
			if (this.curTeam.team.length < 6 && !isAdd) {
				buf += '<button name="addPokemon"><i class="icon-plus"></i></button> ';
			}
			buf += '<span class="pbvTotal"><b>PBV</b><hr />' + pbv.total + '</span>';
			return buf;
		},
